...
| `fetch_soulsville_amenities.py` | Fetches grocery stores, parks, community centers, libraries, churches, schools, and transit (bus stops + routes) from OpenStreetMap via the Overpass API, over the **wide boundary-clip bbox** (same extent as the boundary layers, so pins don't bunch up in one corner of the map). Grocery includes `shop=convenience`/`greengrocer` and `amenity=marketplace` alongside supermarkets: South Memphis corner stores are tagged that way in OSM, and true supermarkets are rare there (the area is a documented food desert), so a supermarket-only query returns nothing. The `shop`/`amenity` tag is kept on each feature as a category hint. Writes to `frontend/public/memphis/amenities/` and `frontend/public/memphis/transit/`. Known gap: OSM has zero tagged MATA bus stops in this area — `transit/stops.geojson` stays empty until the official MATA dataset is manually downloaded (see below). |
